...
label_test_file, label_file = 'label.dat.test', 'label.dat'
...
def parse_args():
    ...
    parser.add_argument('-supervised', required=True, type=str, help='Only TEDM-PU, PUNE, GCN, R-GCN, HAN, MAGNN, and TED support semi-supervised training.', 
                        choices=['True','False'])
    parser.add_argument('-ratio', required=True, type=str, default='5v5')
...
def main():
    ...
    label_file = 'label_TaxPayer{}.dat'.format(args.ratio)
    print(label_file)
    label_test_file = 'label_TaxPayer{}.dat.test'.format(args.ratio)
    ...
    if args.supervised == 'True':
        if args.dataset == 'T20H' or args.dataset == 'T15S':
            emb_file += '_Sup_{}'.format(args.ratio)
        else:
            emb_file += '_Sup'
    else:
        emb_file += '_UnSup'
    emb_file += '.emb.dat'
    print('embedding file name', emb_file)
